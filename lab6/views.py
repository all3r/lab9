...
def table(request, path):
    ...
    match path:
        case 'users':
            ...
            for product in out:
                products.append((product.id, product.email, product.name, product.surname, product.password, product.group_owner, product.admin))
            names = ['id', 'email', 'name', 'surname', 'password', 'group_owner', 'admin', '']
        case 'groups':
            out = Group.objects.all()
            for product in out:
                products.append((product.id, product.author.id, product.name))
            names = ['id', 'author_id', 'name', '']
        case 'posts':
            out = Post.objects.all()
            for product in out:
                products.append((product.id, product.author.id, product.group.id, product.title, product.text, product.date, product.likes))
            names = ['id', 'author_id', 'group_id', 'title', 'text', 'date', 'likes', '']
        case 'comms':
            out = Comm.objects.all()
            for product in out:
                products.append((product.id, product.author.id, product.post.id, product.text, product.date, product.likes))
            names = ['id', 'author_id', 'post_id', 'text', 'date', 'likes', '']
        case 'emojis':
            out = Emoji.objects.all()
            for product in out:
                products.append((product.id, product.code, product.value))
            names = ['id', 'code', 'value', '']
        case 'post_emojis':
            out = Post_Emoji.objects.all()
            for product in out:
                products.append((product.id, product.post.id, product.emoji.id, product.user.id))
            ...
            match path:
                case 'users':
                    ...
                    if form.is_valid():
                        ...
                        user = User(name=name, surname=surname, group_owner=bool(group_owner), email=email, password=password, admin=bool(admin), username=email)
                        user.save()
                case 'groups':
